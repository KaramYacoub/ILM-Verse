...
function ParentShowAbsences() {
  ...
  const applyFilters = () => {
    ...
    if (selectedTimePeriod === "This Semester") {
      ...
      timePeriodFiltered = allRecords.filter((record) => {
        const recordDate = new Date(record.date.split("/").reverse().join("-"));
        return (
          recordDate >= new Date("2024-07-01") &&
    ...
    if (selectedType === "Excused") {
      typeFiltered = timePeriodFiltered.filter(
        (record) => record.status === "Excused"
      );
    } else if (selectedType === "Unexcused") {
      typeFiltered = timePeriodFiltered.filter(
        (record) => record.status === "Unexcused"
      );
    }
  ...
  const filteredExcused = filteredRecords.filter(
    (record) => record.status === "Excused"
  ).length;
  const filteredUnexcused = filteredRecords.filter(
    (record) => record.status === "Unexcused"
  ).length;
      ...
      <div className="p-6 space-y-6 w-full max-w-4xl bg-base-100 rounded-lg shadow-md mt-5">
        ...
        <div className="grid grid-cols-3 gap-6 mt-4">
          <div className="bg-base-200 p-4 rounded-lg text-center">
            ...
            <p className="text-2xl font-bold text-primary">{filteredTotal}</p>
